...
describe('Sprint 5: Admin Contact Management', () => {
  ...
  describe('PUT /api/contact/admin/:id/status', () => {
    it('should update enquiry status', async () => {
      const res = await request(app)
        .put(`/api/contact/admin/${enquiryId}/status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ status: 'read' });
    ...
    it('should allow isRead update', async () => {
      const res = await request(app)
        .put(`/api/contact/admin/${enquiryId}/status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ isRead: true });
    ...
    it('should allow adminNote update', async () => {
      const res = await request(app)
        .put(`/api/contact/admin/${enquiryId}/status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ adminNote: 'Follow up needed' });

      expect(res.status).toBe(200);
      expect(res.body.data.adminNote).toBe('Follow up needed');
    });

    it('should return 404 for non-existent enquiry', async () => {
      const fakeId = new mongoose.Types.ObjectId().toString();
      const res = await request(app)
        .put(`/api/contact/admin/${fakeId}/status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ status: 'read' });
    ...
    it('should return 400 for invalid ID', async () => {
      const res = await request(app)
        .put('/api/contact/admin/invalid-id/status')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ status: 'read' });
  ...
  describe('POST /api/contact/admin/:id/reply', () => {
    it('should send reply email successfully', async () => {
      const enquiry = await createEnquiry('read');
      const res = await request(app)
        .post(`/api/contact/admin/${enquiry._id}/reply`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ replyMessage: 'Thank you for your enquiry' });

      expect([200, 503]).toContain(res.status);
    });

    it('should reject empty reply message', async () => {
      const enquiry = await createEnquiry('read');
      const res = await request(app)
        .post(`/api/contact/admin/${enquiry._id}/reply`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('should return 404 for non-existent enquiry', async () => {
      const fakeId = new mongoose.Types.ObjectId().toString();
      const res = await request(app)
        .post(`/api/contact/admin/${fakeId}/reply`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('should return 400 for invalid ID', async () => {
      const res = await request(app)
        .post('/api/contact/admin/invalid-id/reply')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ replyMessage: 'Test reply' });
    ...
    it('should reject non-admin users', async () => {
      const res = await request(app)
        .post(`/api/contact/admin/${enquiryId}/reply`)
        .set('Authorization', `Bearer ${userToken}`)
        .send({ replyMessage: 'Test reply' });
